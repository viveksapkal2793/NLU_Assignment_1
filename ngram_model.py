...
def extract_mixed_ngrams(words, max_n=3):
    """Extract mixed n-grams from 1 to max_n (combines unigrams, bigrams, trigrams)."""
    all_ngrams = []
    for n in range(1, min(max_n + 1, len(words) + 1)):
        if n == 1:
            # For unigrams, just add the words directly
...
def train_ngram_naive_bayes_mixed(train_data, max_n=3):
    ...
    for text, label in train_data:
        class_counts[label] += 1
        words = tokenize(text)
        ngrams = extract_mixed_ngrams(words, max_n)
...
def ngram_features_mixed(text, vocab_idx, max_n=3):
...
def train_ngram_logistic_regression_mixed(train_data, max_n=3, learning_rate=0.1, epochs=20):
    """Train logistic regression with mixed N-gram features."""
    # Build vocabulary of mixed n-grams
    vocab = set()
    for text, _ in train_data:
        words = tokenize(text)
        ngrams = extract_mixed_ngrams(words, max_n)
    ...
    return weights, bias, vocab_idx, max_n, label_map


def predict_ngram_logistic_regression_mixed(text, model):
    """Predict using trained mixed n-gram logistic regression model."""
...
def predict_ngram_svm_mixed(text, model):
    """Predict using trained mixed n-gram SVM model."""
    weights, bias, vocab_idx, max_n, label_map = model
    features = ngram_features_mixed(text, vocab_idx, max_n)
...
# ================== K-NEAREST NEIGHBORS (MIXED N-GRAMS) ==================

def train_ngram_knn_mixed(train_data, max_n=3):
    """KNN with mixed n-grams."""
    vocab = set()
    for text, _ in train_data:
        words = tokenize(text)
        ngrams = extract_mixed_ngrams(words, max_n)
...
def predict_ngram_knn_mixed(text, model, k=5):
    """Predict using KNN with mixed n-grams."""
